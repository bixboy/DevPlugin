...
void UUnitFormationComponent::BuildFormationCommands(const FCommandData& BaseCommand, const TArray<AActor*>& Units, TArray<FCommandData>& OutCommands)
{
    ...
    if (FormationData && FormationData->SlotOffsets.Num() >= Count)
    {
        ...
        {
            const FVector2D& P = FormationData->SlotOffsets[i];
            Offsets.Add(FVector(FormationData->Offset.X + P.X * FormationSpacing, FormationData->Offset.Y + P.Y * FormationSpacing, FormationData->Offset.Z));
        }
        bHandledByLib = true;
    }
    else
    {
        ...
        switch (CurrentFormation)
        {
            case EFormation::Square:
                UFormationMathLibrary::GetSquareOffsets(Count, FormationSpacing, Offsets);
                bHandledByLib = true;
                break;
        	
            case EFormation::Blob:
                UFormationMathLibrary::GetBlobOffsets(Count, FormationSpacing, Offsets);
                bHandledByLib = true;
                break;
    ...
    if (!bHandledByLib)
    {
        ...
        {
            Offsets.Add(CalculateOffset(Index, Count));
        }
    }
