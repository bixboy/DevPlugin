...
void UPatrolDetailWidget::ClearBinding()
{
    ...
    SetVisibility(ESlateVisibility::Collapsed); // Hide self when nothing selected
}
...
void UPatrolDetailWidget::RefreshUI()
{
    ...
    // Find the route
    const TArray<FPatrolRoute>& Routes = WeakPatrolComponent->GetActiveRoutes();
    for (const FPatrolRoute& R : Routes)
    {
    ...
    if (Input_Name)
    {
        // Only update if not currently focused by user to avoid typing interruption?
...
void UPatrolDetailWidget::OnLoopClicked(UCustomButtonWidget* Button, int Index)
{
    if (WeakPatrolComponent.IsValid())
    {
        // Simple toggle for now: Loop -> OneShot -> Loop (or PingPong if supported later)
        // We need to know current state. Efficient way is to re-find route or cache it.
        // Let's re-find to be safe.
        for (const FPatrolRoute& R : WeakPatrolComponent->GetActiveRoutes())
        {
            if (R.PatrolID == BoundPatrolID)
            {
                EPatrolType NewType = (R.PatrolType == EPatrolType::Loop) ? EPatrolType::Once : EPatrolType::Loop; // Just toggling 2 for simplicity as requested "change to loop" implies toggle or set.
                // Or maybe cycle: Loop -> PingPong -> Once.
                // User said "change la patrol pour quelle fasse une loop". Maybe just Set to Loop? 
                // But usually we want to toggle. Let's Cycle: Loop -> PingPong -> Once -> Loop.
                
                if (R.PatrolType == EPatrolType::Loop) NewType = EPatrolType::PingPong;
                else if (R.PatrolType == EPatrolType::PingPong) NewType = EPatrolType::Once;
                else NewType = EPatrolType::Loop;

                WeakPatrolComponent->Server_SetPatrolType(BoundPatrolID, NewType);
                break;
            }
        }
    }
...
void UPatrolDetailWidget::OnReverseClicked(UCustomButtonWidget* Button, int Index)
{
    if (WeakPatrolComponent.IsValid())
    {
        WeakPatrolComponent->Server_ReversePatrolRouteByID(BoundPatrolID);
    }
}
...
{
	DeletePatrol();
    ClearBinding(); // Close detail view after delete
}
...
void UPatrolDetailWidget::OnNameCommitted(const FText& Text, ETextCommit::Type CommitMethod)
{
    if (CommitMethod == ETextCommit::OnEnter || CommitMethod == ETextCommit::OnUserMovedFocus)
    {
        if (WeakPatrolComponent.IsValid())
        {
            WeakPatrolComponent->Server_RenamePatrol(BoundPatrolID, FName(*Text.ToString()));
        }
    }
}

// Logic migrated from PatrolEntryWidget
void UPatrolDetailWidget::FocusCamera()
{
    if (!WeakPatrolComponent.IsValid()) return;

    // Find route to get points
    FPatrolRoute Route;
    bool bFound = false;
    for (const FPatrolRoute& R : WeakPatrolComponent->GetActiveRoutes())
    {
    ...
    if (!bFound || Route.PatrolPoints.Num() == 0) return;
