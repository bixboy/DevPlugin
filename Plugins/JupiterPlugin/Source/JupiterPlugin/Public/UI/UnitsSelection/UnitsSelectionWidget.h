...
UENUM(BlueprintType)
enum class EUnitSelectionPage : uint8
{
    Spawn = 0,
    Patrol = 1
};
...
public:
    ...
    void Init(UUnitSpawnComponent* InSpawnComponent, UUnitPatrolComponent* InDefaultPatrolComponent);

    void SetSelectionContext(const TArray<AActor*>& SelectedActors);
...
protected:
    ...
    UFUNCTION()
    void SwitchToPage(EUnitSelectionPage Page);
    ...
    UPROPERTY()
    TWeakObjectPtr<UUnitPatrolComponent> DefaultPatrolComponent;
};
